...
export async function POST(request){
  ...
  try{
    ...
    if(preferred){
      const {data:existing}=await db.from('designs').select('id,status,created_at').eq('id',preferred).eq('customer_phone',phone).maybeSingle();
      if(existing?.status==='processing')return Response.json({id:existing.id,status:existing.status,createdAt:existing.created_at});
    }
