...
export default function Home(){
  ...
  useEffect(()=>{setStageZoom({scale:1,x:0,y:0});pinch.current=null;drag.current=null;},[activeView,productId]);
  ...
  function move(e){if(!drag.current||!stage.current||pinch.current)return;const r=stage.current.getBoundingClientRect();const x=(e.clientX-r.left-stageZoom.x)/(r.width*stageZoom.scale),y=(e.clientY-r.top-stageZoom.y)/(r.height*stageZoom.scale);setOrder(null);setPlaced(a=>a.map(item=>item.uid===drag.current?{...item,x:Math.max(.08,Math.min(.92,x)),y:Math.max(.08,Math.min(.92,y))}:item));}
