...
export default function Home(){
  ...
  const giftCandidates=(configuredGiftPatches.length?configuredGiftPatches:catalog.patches.slice(0,2).map(p=>p.id)).map(id=>catalog.patches.find(p=>p.id===id)).filter(Boolean).slice(0,2);
  ...
  function claimGift(p){
    if(!p)return;
    setGiftPatchId(p.id);setChosen(p.id);setActivePatchGroup('all');setGiftPopupOpen(false);flashQuote(p);
  ...
  useEffect(()=>{if(!catalogReady||!giftOffer.enabled||giftCandidates.length<2)return;setGiftPopupOpen(true);},[catalogReady]);
