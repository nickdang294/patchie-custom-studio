...
export default function Home(){
  ...
  const BaseConfig=()=> {const baseReady=Boolean(productId&&product?.color&&size);return <section className={`base-config${baseCollapsed?' is-collapsed':''}`} aria-label="Cấu hình sản phẩm base"><div className="base-config-heading"><div><small>SẢN PHẨM BASE</small><b>{baseCollapsed?'Đã chọn sản phẩm':'Chọn sản phẩm, màu và size'}</b></div>{baseCollapsed&&<button type="button" className="base-config-toggle is-edit" onClick={()=>setBaseCollapsed(false)}>Chỉnh sửa <span>⌄</span></button>}</div>{baseCollapsed?<div className="base-config-summary"><img src={productThumb(product)} alt=""/><div><b>{product?.name||'Sản phẩm base'}</b><small>{product?.color||'Chưa chọn màu'}{size?` · Size ${size}`:''}</small></div><strong>{money(product?.price)}</strong></div>:<div className="base-config-fields"><ProductSelect note/>{colorVariants.length>1&&<ColorSelect/>}{sizes.length>1&&<SizeSelect/>}{baseReady&&<button type="button" className="choose-patch-cta" onClick={()=>setBaseCollapsed(true)}>Chọn patch <span>→</span></button>}</div>}</section>};
