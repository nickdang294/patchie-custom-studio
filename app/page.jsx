...
export default function Home(){
  ...
  const productGroupOptions=PRODUCT_GROUPS.map(g=>{const list=productsByGroup(g.value),first=list[0]||catalog.products[0]||defaultProduct;return {value:g.value,label:g.label,sub:`${productBasesByGroup(g.value).length} mẫu`,image:productThumb(first)};});
  ...
  const ColorSelect=()=> <div className="form-field color-field"><label>Màu</label><div className="color-row">{colorVariants.map(v=><button type="button" key={v.id} className={`color-chip ${v.id===productId?'on':''}`} onClick={()=>changeProductColor(v.id)}><span style={{background:v.hex||'#f5f1e8'}}></span><b>{v.color}</b></button>)}</div></div>;
