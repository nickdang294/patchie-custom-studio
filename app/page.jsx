...
export default function Home(){
  ...
  const stage=useRef(null),drag=useRef(null),pan=useRef(null),rotating=useRef(false),pinch=useRef(null);
  ...
  useEffect(()=>{setStageZoom({scale:1,x:0,y:0});pinch.current=null;drag.current=null;pan.current=null;},[activeView,productId]);
  ...
  function removePatch(uid,e){e?.preventDefault();e?.stopPropagation();remember();setPlaced(a=>a.filter(x=>x.uid!==uid));if(selectedUid===uid)setSelectedUid('');show('Đã xoá patch.');}
  ...
  function startDrag(e,uid){e.preventDefault();e.stopPropagation();setSelectedUid(uid);remember();pan.current=null;drag.current=uid;e.currentTarget.setPointerCapture(e.pointerId);}
  ...
  function endStage(){drag.current=null;pan.current=null;}
