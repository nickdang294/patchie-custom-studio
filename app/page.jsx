...
const STARTER=[
  {id:'patch-pink',name:'Mũ xanh lá',image_url:'/assets/patch-pink-cap.webp',width_cm:4,height_cm:4,quote:'nhỏ xíu mà có võ',patch_group:'Best Seller'},
  {id:'patch-black',name:'Mặt nạ xanh',image_url:'/assets/patch-black-green.webp',width_cm:4,height_cm:4,quote:'bí ẩn một chút mới vui',patch_group:'Limited'},
  {id:'patch-red',name:'Mũ vàng',image_url:'/assets/patch-red-white.webp',width_cm:4,height_cm:4,quote:'đội mood vui lên áo',patch_group:'Seasonal'},
  {id:'patch-yellow',name:'Mũ xanh dương',image_url:'/assets/patch-yellow-blue.webp',width_cm:4,height_cm:4,quote:'hôm nay hơi đáng yêu',patch_group:'Cute Animal'}
];

const PATCH_GROUPS=['Best Seller','Chữ Cái','Seasonal','Limited','Cute Animal','Floral'];
const patchGroup=p=>String(p?.patch_group||'Best Seller').trim()||'Best Seller';
...
export default function Home(){
  ...
  const patchGroups=['all',...Array.from(new Set([...PATCH_GROUPS,...catalog.patches.map(patchGroup)]))];
  const visiblePatches=activePatchGroup==='all'?catalog.patches:catalog.patches.filter(p=>patchGroup(p)===activePatchGroup);
