...
export default function Home(){
  const [catalog,setCatalog]=useState({products:[defaultProduct],patches:STARTER,settings:{sizes:['S','M','L','XL'],messengerUrl:'',privacyText:'Shop dùng thông tin này để xử lý yêu cầu thiết kế.',brand:BRAND_DEFAULTS,giftOffer:GIFT_DEFAULTS,popupCampaigns:normalizePopupCampaigns(null,GIFT_DEFAULTS),activePopupCampaign:'traditional-gift',defaultPatchGroup:'all',halloweenThemeEnabled:true}});
  ...
  const halloweenThemeEnabled=catalog.settings.halloweenThemeEnabled!==false;
  ...
  useEffect(()=>{fetch('/api/catalog').then(r=>r.json()).then(d=>{const products=Array.isArray(d.products)?d.products:[],patches=Array.isArray(d.patches)?d.patches:[],livePatches=patches.length?patches:STARTER,settings={sizes:['S','M','L','XL'],messengerUrl:'',privacyText:'Shop dùng thông tin này để xử lý yêu cầu thiết kế.',giftOffer:GIFT_DEFAULTS,popupCampaigns:normalizePopupCampaigns(null,GIFT_DEFAULTS),activePopupCampaign:'traditional-gift',defaultPatchGroup:'all',halloweenThemeEnabled:true,...(d.settings||{})},availableGroups=new Set(livePatches.flatMap(patchGroupsOf)),requestedGroup=String(settings.defaultPatchGroup||'all'),initialGroup=requestedGroup==='all'||availableGroups.has(requestedGroup)?requestedGroup:'all';setCatalog({products,patches:livePatches,settings});setActivePatchGroup(initialGroup);if(products.length)setProductId(products[0].id);if(livePatches.length)setChosen(livePatches[0].id);}).catch(()=>{}).finally(()=>setCatalogReady(true));},[]);
  ...
  if(!catalogReady)return <main className={`shell ${halloweenThemeEnabled?'theme-halloween':''}`} style={{'--patchie-font':brand.fontFamily}}><StudioHeader brand={brand}/><LoadingAnimation/><StudioFooter/></main>;
  if(!catalog.products.length)return <main className={`shell ${halloweenThemeEnabled?'theme-halloween':''}`} style={{'--patchie-font':brand.fontFamily}}><StudioHeader brand={brand}/><section className="catalog-loading"><b>Chưa có sản phẩm base</b><p>Vào admin để thêm sản phẩm hoặc màu variation trước nhé.</p></section><StudioFooter/></main>;
  ...
  return <main className={`shell ${halloweenThemeEnabled?'theme-halloween':''}`} style={{'--patchie-font':brand.fontFamily,'--step-base-desktop':cssContent(brand.stepBaseDesktop),'--step-base-mobile':cssContent(brand.stepBaseMobile),'--step-patch-desktop':cssContent(brand.stepPatchDesktop),'--step-patch-mobile':cssContent(brand.stepPatchMobile),'--step-preview-desktop':cssContent(brand.stepPreviewDesktop),'--step-preview-mobile':cssContent(brand.stepPreviewMobile),'--step-order-desktop':cssContent(brand.stepOrderDesktop),'--step-order-mobile':cssContent(brand.stepOrderMobile),'--step1-desktop':cssContent(brand.stepBaseDesktop),'--step1-mobile':cssContent(brand.stepBaseMobile),'--step2-desktop':cssContent(brand.stepPatchDesktop),'--step2-mobile':cssContent(brand.stepPatchMobile)}}><StudioHeader brand={brand}/>
