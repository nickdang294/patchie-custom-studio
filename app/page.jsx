...
export default function Home(){
  const [catalog,setCatalog]=useState({products:[defaultProduct],patches:STARTER,settings:{sizes:['S','M','L','XL'],messengerUrl:'',privacyText:'Shop dùng thông tin này để xử lý yêu cầu thiết kế.',brand:BRAND_DEFAULTS,giftOffer:GIFT_DEFAULTS,defaultPatchGroup:'all'}});
  ...
  const configuredGiftPatches=Array.isArray(giftOffer.patchIds)?Array.from(new Set(giftOffer.patchIds)):[];
  ...
  function claimGift(p){
    if(!p)return;
    setGiftPatchId(p.id);setGiftPreviewId('');setChosen(p.id);setActivePatchGroup('all');setGiftPopupOpen(false);flashQuote(p);
  ...
  useEffect(()=>{fetch('/api/catalog').then(r=>r.json()).then(d=>{const products=Array.isArray(d.products)?d.products:[],patches=Array.isArray(d.patches)?d.patches:[],livePatches=patches.length?patches:STARTER,settings={sizes:['S','M','L','XL'],messengerUrl:'',privacyText:'Shop dùng thông tin này để xử lý yêu cầu thiết kế.',giftOffer:GIFT_DEFAULTS,defaultPatchGroup:'all',...(d.settings||{})},availableGroups=new Set(livePatches.flatMap(patchGroupsOf)),requestedGroup=String(settings.defaultPatchGroup||'all'),initialGroup=requestedGroup==='all'||availableGroups.has(requestedGroup)?requestedGroup:'all';setCatalog({products,patches:livePatches,settings});setActivePatchGroup(initialGroup);if(products.length)setProductId(products[0].id);if(livePatches.length)setChosen(livePatches[0].id);}).catch(()=>{}).finally(()=>setCatalogReady(true));},[]);
  ...
  const SizeSelect=()=> <div className="form-field size-field"><label>Size</label><div className="size-row">{sizes.map(s=><button type="button" key={s} className={`size ${size===s?'on':''}`} onClick={()=>{setSize(s);if(productId&&product?.color)setBaseCollapsed(true);}}>{s}</button>)}</div>{size&&String(product?.size_guide||'').trim()&&<small className="size-guide-note">{String(product.size_guide).trim()}</small>}</div>;
  const BaseConfig=()=> <section className={`base-config${baseCollapsed?' is-collapsed':''}`} aria-label="Cấu hình sản phẩm base"><div className="base-config-heading"><div><small>SẢN PHẨM BASE</small><b>{baseCollapsed?'Đã chọn sản phẩm':'Chọn sản phẩm, màu và size'}</b></div><button type="button" className="base-config-toggle" onClick={()=>setBaseCollapsed(value=>!value)}>{baseCollapsed?'Chỉnh sửa':'Thu gọn'} <span>{baseCollapsed?'⌄':'⌃'}</span></button></div>{baseCollapsed?<div className="base-config-summary"><img src={productThumb(product)} alt=""/><div><b>{product?.name||'Sản phẩm base'}</b><small>{product?.color||'Chưa chọn màu'}{size?` · Size ${size}`:''}</small></div><strong>{money(product?.price)}</strong></div>:<div className="base-config-fields"><ProductSelect note/><ColorSelect/><SizeSelect/></div>}</section>;
