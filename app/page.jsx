...
const BrandIdentity=({brand=BRAND_DEFAULTS})=><>{brand.logoUrl?<img className="brand-logo" src={brand.logoUrl} alt=""/>:<span className="brand-mark">P</span>} {brand.brandName||'Patchie'}<span className="brand-studio">STUDIO</span><span className="brand-market-link" role="link" tabIndex="0" onClick={event=>{event.preventDefault();event.stopPropagation();window.location.href='/patches';}} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();window.location.href='/patches';}}}>Patch Market ↗</span></>;
...
export default function Home(){
  ...
  if(!catalogReady)return <main className="shell" style={{'--patchie-font':brand.fontFamily}}><header className="topbar"><a className="brand" href="/"><BrandIdentity brand={brand}/></a><div className="toplinks"><span>Sản phẩm base · Patch thêu ủi</span><a className="market-nav-link" href="/patches">Patch Market ↗</a><a href="/admin">Quản trị ↗</a></div></header><LoadingAnimation/></main>;
  if(!catalog.products.length)return <main className="shell" style={{'--patchie-font':brand.fontFamily}}><header className="topbar"><a className="brand" href="/"><BrandIdentity brand={brand}/></a><div className="toplinks"><span>Sản phẩm base · Patch thêu ủi</span><a className="market-nav-link" href="/patches">Patch Market ↗</a><a href="/admin">Quản trị ↗</a></div></header><section className="catalog-loading"><b>Chưa có sản phẩm base</b><p>Vào admin để thêm sản phẩm hoặc màu variation trước nhé.</p></section></main>;
