...
export default function Home(){
  ...
  const flashQuote=(text,ms=1000)=>{if(!text)return;setQuote(text);setTimeout(()=>setQuote(null),ms);};
  const choosePatch=p=>{setChosen(p.id);flashQuote(p.quote||'',1000);};
  ...
  function removeDraggedPatch(uid){setTrashQuote(true);setTimeout(()=>setTrashQuote(false),900);setPlaced(a=>a.filter(x=>x.uid!==uid));if(selectedUid===uid)setSelectedUid('');}
