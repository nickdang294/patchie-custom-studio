...
export default function Home(){
  ...
  const productGroupOptions=PRODUCT_GROUPS.map(g=>{const list=productsByGroup(g.value),first=list[0]||catalog.products[0]||defaultProduct;return {value:g.value,label:g.label,sub:`${list.length} mẫu`,image:productThumb(first)};});
  const patchGroupOptions=patchGroups.map(g=>({value:g,label:g==='all'?'Tất cả':g,sub:`${patchGroupCount(g)} patch`}));
  const productOptions=productsByGroup(productSheetGroup).map(p=>({value:p.id,label:p.name,sub:p.color,price:money(p.price),image:productThumb(p)}));
  const selectData=selectSheet==='patchGroup'?{label:'Nhóm patch',intro:'Chọn nhóm patch muốn xem.',value:activePatchGroup,options:patchGroupOptions,onChange:setActivePatchGroup}:selectSheet==='productGroup'?{label:'Nhóm sản phẩm',intro:'Chọn loại base trước, rồi chọn mẫu cụ thể.',value:productSheetGroup,options:productGroupOptions,onChange:setProductSheetGroup,nextSheet:'product'}:selectSheet==='product'?{label:productGroupLabel(productSheetGroup),intro:'Chọn mẫu base muốn custom.',value:productId,options:productOptions,onChange:changeProduct,backSheet:'productGroup'}:null;
  ...
  const ProductSelect=({note=false})=> <div className="form-field product-select-field"><CuteSelect label="Sản phẩm base" value={productId} sheetKey="productGroup" onOpen={()=>setProductSheetGroup(product?.product_type||'shirt')} note={note?`Giá base: ${money(product?.price)}`:''} options={catalog.products.map(p=>({value:p.id,label:p.name,sub:p.color,price:money(p.price),image:productThumb(p)}))}/></div>;
