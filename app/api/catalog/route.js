...
const starter = {
  products: [{ id:'base-offwhite', name:'Áo thun oversized', color:'Off-white', hex:'#f5f1e8', image_url:'/assets/blank-tee.webp', sizes:['S','M','L','XL'], price:null }],
  patches: [
    { id:'patch-pink', name:'Mũ xanh lá', image_url:'/assets/patch-pink-cap.webp', width_cm:1, height_cm:1, price:null },
    { id:'patch-black', name:'Mặt nạ xanh', image_url:'/assets/patch-black-green.webp', width_cm:1, height_cm:1, price:null },
    { id:'patch-red', name:'Mũ vàng', image_url:'/assets/patch-red-white.webp', width_cm:1, height_cm:1, price:null },
    { id:'patch-yellow', name:'Mũ xanh dương', image_url:'/assets/patch-yellow-blue.webp', width_cm:1, height_cm:1, price:null }
  ],
...
export async function GET() {
  try {
    ...
    const [{data:products,error:pErr},{data:patches,error:paErr},{data:settings,error:sErr}] = await Promise.all([
      ...
      db.from('patches').select('*').eq('active',true).order('sort_order'),
