...
export async function GET(request){
  ...
  if(data?.length){await db.storage.from('design-mockups').remove(data.map(x=>x.image_path));const {error:delError}=await db.from('designs').delete().in('id',data.map(x=>x.id));if(delError)return Response.json({error:delError.message},{status:500});}
