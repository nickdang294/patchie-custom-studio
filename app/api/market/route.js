...
export async function GET(){
  try{
    ...
    const [{data:patches,error:pErr},{data:settings,error:sErr}]=await Promise.all([
      ...
      db.from('settings').select('key,value').in('key',['market','messengerUrl','brand','halloweenThemeEnabled'])
    ]);
    if(pErr||sErr)throw new Error('Chưa tải được Patch Market.');
    const config=Object.fromEntries((settings||[]).map(x=>[x.key,x.value]));
    return Response.json({patches:patches||[],market:{...MARKET_DEFAULTS,...(config.market||{})},messengerUrl:config.messengerUrl||'',brand:config.brand||{},halloweenThemeEnabled:config.halloweenThemeEnabled!==false},{headers:{'Cache-Control':'no-store, max-age=0'}});
  }catch(e){
    if(!process.env.NEXT_PUBLIC_SUPABASE_URL||!process.env.SUPABASE_SERVICE_ROLE_KEY)return Response.json({patches:starterPatches,market:MARKET_DEFAULTS,messengerUrl:'',brand:{},halloweenThemeEnabled:true});
